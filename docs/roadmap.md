...
Initial implementation must target `/Users/amir/test_sync` only. Do not run against `/Users/amir/projects` until the test folder workflow is proven.
...
- Keep `/Users/amir/test_sync` as the only target.
